...
class EventItem extends Component {
    ...
    initStartDrag = (ev) => {
        const {eventItem, getEventSlotId, getSlotById, isResizing, startResizing} = this.props;
        let slotId = getEventSlotId(eventItem);
    ...
    doStartDrag = (ev) => {
        ...
        const {left, width, leftIndex, rightIndex, contentCellWidth} = this.props;
    ...
    stopStartDrag = (ev) => {
        ...
        const {cellUnit, events, config, localeMoment, contentCellWidth} = this.props;
        let cellWidth = contentCellWidth;
        let offset = leftIndex > 0 ? 5 : 6;
        let minWidth = cellWidth - offset;
        let maxWidth = rightIndex * cellWidth - offset;
        ...
        if (hasConflict) {
            ...
            if (typeof conflictOccurred !== "undefined") {
                conflictOccurred('StartResize', eventItem, DnDTypes.EVENT, slotId, slotName, newStart, eventItem.end);
            } else {
            ...
            if (typeof updateEventStart !== "undefined")
                updateEventStart(eventItem, newStart);
        }
    }
    ...
    cancelStartDrag = (ev) => {
        ...
        document.onselectstart = null;
        document.ondragstart = null;
        const {stopResizing, left, top, width} = this.props;
        stopResizing();
        this.setState({
            left: left,
            top: top,
    ...
    initEndDrag = (ev) => {
        const {startResizing, isResizing, getEventSlotId, getSlotById, eventItem} = this.props;
        let slotId = getEventSlotId(eventItem);
        let slot = getSlotById(slotId);
        if (!!slot && !!slot.groupOnly) return;
        if (isResizing) return;
    ...
    doEndDrag = (ev) => {
        ...
        const {width, leftIndex, headers, contentCellWidth} = this.props;
        let cellWidth = contentCellWidth;
        let offset = leftIndex > 0 ? 5 : 6;
        let minWidth = cellWidth - offset;
    ...
    stopEndDrag = (ev) => {
        ...
        const {headers, cellUnit, events, config, localeMoment, contentCellWidth} = this.props;
        let cellWidth = contentCellWidth;
        let offset = leftIndex > 0 ? 5 : 6;
        let minWidth = cellWidth - offset;
        let maxWidth = (headers.length - leftIndex) * cellWidth - offset;
        ...
        if (hasConflict) {
            ...
            if (typeof conflictOccurred !== "undefined") {
                conflictOccurred('EndResize', eventItem, DnDTypes.EVENT, slotId, slotName, eventItem.start, newEnd);
            } else {
                console.log('Conflict occurred, set conflictOccurred func in Scheduler to handle it');
            }
            this.subscribeResizeEvent(this.props);
            ...
            if (typeof updateEventEnd !== "undefined")
                updateEventEnd(eventItem, newEnd);
        }
    }
